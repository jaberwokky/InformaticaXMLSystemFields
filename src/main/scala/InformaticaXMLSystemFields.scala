    ...
    object MainConnectionTransform extends RewriteRule {
      override def transform(n: Node): Seq[Node] = n match {
        case sesstl @ Elem(_, "MAPPING", _, _, _*) => {
          val ccd = "\"" + args(1) + "\""
          addChild(addChild(addChild(addChild(sesstl, <MAPPINGVARIABLE DATATYPE ="string" DEFAULTVALUE ={ ccd } DESCRIPTION ="" ISEXPRESSIONVARIABLE ="NO" ISPARAM ="YES" NAME ="$$mpSourceSystemCCD" PRECISION ="3" SCALE ="0" USERDEFINED ="YES"/>),
        ...
        case sessfl @ Elem(_, "TRANSFORMATION", _, _, _*) if {
            val attrs = sessfl.attributes.asAttrMap
            attrs("NAME").toLowerCase.contains("fields")
          } => {
            addChild(addChild(addChild(addChild(sessfl, <TRANSFORMFIELD DATATYPE ="string" DEFAULTVALUE ="" DESCRIPTION ="" EXPRESSION ="$$mpSourceSystemCCD" EXPRESSIONTYPE ="GENERAL" NAME ="SOURCE_SYSTEM_CCD" PICTURETEXT ="" PORTTYPE ="OUTPUT" PRECISION ="3" SCALE ="0"/>),
